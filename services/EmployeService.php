...
class EmployeService implements IDao {
    ...
    public function create($o) {
        $query = "INSERT INTO `employe`(`cin`, `nom`, `prenom`, `email`, `telephone`, `adresse`, `password`, `role`, `photo`) "
                . "VALUES (?,?,?,?,?,?,?,?,?)";
        $req = $this->connexion->getConnexion()->prepare($query);
        $req->execute(array($o->getCin(), $o->getNom(), $o->getPrenom(), $o->getEmail(), $o->getTelephone(), $o->getAdresse(),
                    $o->getPassword(), $o->getRole(), $o->getPhoto())) or die('Error');
    }
    ...
    public function findById($cin) {
        ...
        $employe = new Employe($res->cin, $res->nom, $res->prenom, $res->email, $res->telephone, $res->adresse, $res->password, $res->role, $res->photo);
        return $employe;
    }
    ...
    public function update($o) {
        $query = "UPDATE Employe SET  nom=?, prenom =?, email =?, telephone =?, adresse =?, password =?, role =?, photo =? where cin = ?";
        $req = $this->connexion->getConnexion()->prepare($query);
        $req->execute(array($o->getNom(), $o->getPrenom(), $o->getEmail(), $o->getTelephone(), $o->getAdresse(),
                    $o->getPassword(), $o->getRole(), $o->getPhoto(), $o->getCin())) or die('Error');
    }
